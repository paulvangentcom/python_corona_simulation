...
def update(frame, population, destinations, pop_size, infection_range=0.01, 
    ...
    #find new infections
    population = Infection.Infection_locator.infect(population, pop_size, infection_range, infection_chance, frame, 
                        healthcare_capacity, verbose)
    infected_plot.append(len(population[population[:,6] == 1]))

    #recover and die
    population = Infection.Infection_computator.recover_or_die(population, frame, recovery_duration, mortality_chance,
