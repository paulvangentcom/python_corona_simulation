...
def update(
    ...
    if active_dests > 0 and len(population[population[:, 12] == 1]) > 0:
        ...
        population = keep_at_destination(population, destinations, wander_factor=1)
    ...
    if visualise:
        ...
        ax1.scatter(
            fatalities[:, 0], fatalities[:, 1], color="black", s=2, label="fatalities"
        )
        ...
        if treatment_dependent_risk:
            ...
            ax2.plot(
                indices, infected_arr[infected_arr >= healthcare_capacity], color="red"
            )
...
if __name__ == "__main__":
    ...
    # illness parameters
    infection_range = (
        0.01  # range surrounding infected patient that infections can take place
    )
    infection_chance = (
    ...
    animation = FuncAnimation(
        fig, update, fargs=fargs, frames=simulation_steps, interval=33
    )
    plt.show()
