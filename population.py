...
def initialize_population(Config, mean_age = 45, max_age = 105,
    ...
    population = np.zeros((Config.get_pop_size(), 15))

    #initalize unique IDs
    population[:,0] = [x for x in range(Config.get_pop_size())]
    ...
    population[:,5] = np.random.normal(Config.get_speed(), Config.get_speed() / 3)
    ...
    population[:,9] = np.random.normal(loc = 0.5, scale = 0.5 / 3, size=(Config.get_pop_size(),))
