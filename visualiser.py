...
def set_style(Config):
    ...
    '''
    if Config.get_plot_style().lower() == 'dark':
...
def build_fig(Config, figsize=(5,7)):
    ...
    plt.xlim(Config.get_xbounds()[0], Config.get_xbounds()[1])
    plt.ylim(Config.get_ybounds()[0], Config.get_ybounds()[1])

    ax2 = fig.add_subplot(spec[1,0])
    ax2.set_title('number of infected')
    #ax2.set_xlim(0, simulation_steps)
    ax2.set_ylim(0, Config.get_pop_size() + 100)
...
def draw_tstep(Config, population, pop_tracker, frame,
    ...
    ax1.set_xlim(Config.get_x_plot()[0], Config.get_x_plot()[1])
    ax1.set_ylim(Config.get_y_plot()[0], Config.get_y_plot()[1])

    if Config.get_self_isolate() and Config.get_isolation_bounds() != None:
        build_hospital(Config.get_isolation_bounds()[0], Config.get_isolation_bounds()[2],
                       Config.get_isolation_bounds()[1], Config.get_isolation_bounds()[3], ax1,
                       addcross = False)
    ...
    ax1.text(Config.get_x_plot()[0],
             Config.get_y_plot()[1] + ((Config.get_y_plot()[1] - Config.get_y_plot()[0]) / 100),
    ...
    ax2.set_title('number of infected')
    ax2.text(0, Config.get_pop_size() * 0.05,
                'https://github.com/paulvangentcom/python-corona-simulation',
                fontsize=6, alpha=0.5)
    #ax2.set_xlim(0, simulation_steps)
    ax2.set_ylim(0, Config.get_pop_size() + 200)

    if Config.get_treatment_dependent_risk():
        infected_arr = np.asarray(pop_tracker.infectious)
        indices = np.argwhere(infected_arr >= Config.get_healthcare_capacity())

        ax2.plot([Config.get_healthcare_capacity() for x in range(len(pop_tracker.infectious))],
                 'r:', label='healthcare capacity')

    if Config.get_plot_mode().lower() == 'default':
        ax2.plot(pop_tracker.infectious, color=palette[1])
        ax2.plot(pop_tracker.fatalities, color=palette[3], label='fatalities')
    elif Config.get_plot_mode().lower() == 'sir':
    ...
    if Config.get_save_plot():
        try:
            plt.savefig('%s/%i.png' %(Config.get_plot_path(), frame))
        except:
            check_folder(Config.get_plot_path())
            plt.savefig('%s/%i.png' %(Config.get_plot_path(), frame))
