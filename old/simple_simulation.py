...
def initialize_population(
    pop_size, mean_age=45, max_age=105, xbounds=[0, 1], ybounds=[0, 1]
):
    ...
    population[:, 1] = np.random.uniform(
        low=xbounds[0] + 0.05, high=xbounds[1] - 0.05, size=(pop_size,)
    )
    population[:, 2] = np.random.uniform(
        low=ybounds[0] + 0.05, high=ybounds[1] - 0.05, size=(pop_size,)
    )
    ...
    population[:, 7] = np.int32(
        np.random.normal(loc=mean_age, scale=std_age, size=(pop_size,))
    )
...
def update_randoms(population, heading_update_chance=0.02, speed_update_chance=0.02):
    ...
    population[:, 3][update <= heading_update_chance] = np.random.normal(
        loc=0, scale=1 / 3, size=shp
    )
    # y
    update = np.random.random(size=(pop_size,))
    shp = update[update <= heading_update_chance].shape
    population[:, 4][update <= heading_update_chance] = np.random.normal(
        loc=0, scale=1 / 3, size=shp
    )
    # randomize speeds
    update = np.random.random(size=(pop_size,))
    shp = update[update <= heading_update_chance].shape
    population[:, 5][update <= heading_update_chance] = np.random.normal(
        loc=0.01, scale=0.01 / 3, size=shp
    )
    return population
...
def infect(population, infection_range, infection_chance, frame):
    ...
    if len(infected_previous_step) < (pop_size // 2):
        for patient in infected_previous_step:
            ...
            infection_zone = [
                patient[1] - infection_range,
                patient[2] - infection_range,
                patient[1] + infection_range,
                patient[2] + infection_range,
            ]
        ...
        for person in healthy_previous_step:
            ...
            infection_zone = [
                person[1] - infection_range,
                person[2] - infection_range,
                person[1] + infection_range,
                person[2] + infection_range,
            ]
            ...
            if (
                ...
                poplen = len(
                    sick_previous_step[:, 6][
                        (infection_zone[0] < sick_previous_step[:, 1])
...
def recover_or_die(population, frame, recovery_duration, mortality_chance):
    ...
    recovery_odds_vector = (illness_duration_vector - recovery_duration[0]) / np.ptp(
        recovery_duration
    )
...
def update(
    ...
    if visualise:
        ...
        ax1.scatter(
            immune[:, 0], immune[:, 1], color=personStateColors[2], s=2, label="immune"
        )
...
if __name__ == "__main__":
    ...
    animation = FuncAnimation(
        fig, update, fargs=(population,), frames=simulation_steps, interval=33
    )
    plt.show()
